...
- 이 자료형은 문자열로 변환하거나, 문자열을 이 자료형으로 변환하는 것만 호환
...
- toString 메서드는 저장하고 있는 문자열을 리턴하도록 만들어져 바로 출력 가능
...
Java에서는 산술 연산의 최소 단위가 int  
 ...
 2개의 서로 다른 숫자 자료형끼리 연산을 하면 큰 자료형으로 변환해  
...
- 강제 형 변환  
  ...
  숫자 데이터끼리 강제 형 변환이 가능, 인스턴스끼리는 상속 관계인 경우만 가능

  - 숫자 데이터와 문자 데이터끼리는 안되지만 메서드 제공

  문자열을 숫자 데이터로 변호나할 때는 Wrapper 클래스 이용
