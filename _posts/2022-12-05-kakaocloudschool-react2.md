...
클래스형 컴포넌트에서는 setState  
함수형 컴포넌트에서는 usetState 함수 이용 필수

객체 나 배열 수정 시 복사본을 만들고 수정 필요
...
클래스 안에 일반 메서드 생성 시, 인스턴스의 메서드로 자동 변형
...
이벤트 처리 함수 매개변수의 target 속성을 호출하면 이벤트 발생한 객체의 참조 리턴
